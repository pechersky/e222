  ...
  { rintros ⟨hA, hB⟩,
    ...
    { ext i j,
      rw [matrix.mul_eq_mul, matrix.mul_val, subsingleton.elim i z, subsingleton.elim j z,
          matrix.one_val_eq, univ_unique, finset.sum_singleton, hz, hB],
      rw [mul_div_cancel' 1 nonzero] <|> rw [mul_comm, mul_div_cancel' 1 nonzero] } },
  { intro h,
    have nonzeroA : A ≠ 0,
    ...
    { have nz : A z z ≠ 0,
        { intros h',
          refine nonzeroA _,
          ext i j,
          rw [matrix.zero_val, subsingleton.elim i z, subsingleton.elim j z, h'] },
      refine (eq_div_iff nz).mpr _,
      replace h := congr_fun (congr_fun h.right z) z,
      rwa [matrix.mul_eq_mul, matrix.mul_val, matrix.one_val_eq, univ_unique,
           finset.sum_singleton, hz] at h,
     } }
end